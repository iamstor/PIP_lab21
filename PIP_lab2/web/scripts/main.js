...
for (let button of document.getElementsByClassName("r-button")) {
    button.addEventListener("click", rCheckBox)
}
...
function xButtonsOnClick(event) {
    ...
    for (let button of document.getElementsByClassName("x-button")) {
        ...
        button.style.background="#8b687a;";
    }
    event.target.classList.add("pressed");
    event.target.style.background="#8b687a;";
...
function rCheckBox(event) {

    rField.value = ""+event.target.value;
    checkR(event);
    let count = 0;
    let checkings = document.forms["form"]["R"];
    for (let i of rField) {
        if (i.checked) {
            count += 1;
        }

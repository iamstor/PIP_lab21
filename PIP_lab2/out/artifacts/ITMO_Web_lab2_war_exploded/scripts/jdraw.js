...
function drawArea(r) {
    let height = canvas.height;
    let width = canvas.width;
    ctx.strokeStyle = "#34136e";
    ctx.fillStyle = "#34136e";
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.beginPath();
    ctx.moveTo(width / 2, height / 2);
    ctx.arc(width / 2, height / 2, r * i / 2, Math.PI / 1, -Math.PI / 2, false);
    ctx.lineTo(width / 2, height / 2 -r * i);
    ctx.lineTo(width / 2 + r * i / 2, height / 2 - r * i);
...
function drawPoint(x, y, color) {
    ctx.fillStyle = (color === "Yes" ? "#008356" : "#ff0070");
    ctx.beginPath();
